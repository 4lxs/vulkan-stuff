...
std::unique_ptr<Scene> Scene::load(std::string_view filePath) {
  Engine& engine = Engine::instance();
  ...
  //
  // scene.descriptorPool.init(engine->_device, gltf.materials.size(), sizes);
  ...
  for (fastgltf::Image& image : gltf.images) {
    ...
    if (img.has_value()) {
      ...
      // imageIDs.push_back(
      // engine->texCache.AddTexture(materialResources.colorImage.imageView,
